...
if(isset($args[$reg_str])){
	if(isset($args['species'])){
		...
		if($args['species'] == 'Rattus norvegicus'){
			...
			$ex1 =  get_loci_from_sql($database1, $qtldb, 'userinterval', $chromosomsEx1, $confidence_int, $intervalStart, $intervalEnd);
			if (!empty($ex1)) {
			...
			$ex2 =  get_loci_from_sql($database2, $qtldb, 'wholeGenome', $chromosomsEx2, $confidence_int);
