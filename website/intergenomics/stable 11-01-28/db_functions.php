...
function getGroupSyntenyIDs($db, $bp, $dnafrag, $dnafrag2name, $species_name){
	...
	/*$sqlDnafrag = 'select synteny_region_id	from dnafrag_region
	 where dnafrag_start <='.$bp['end'].'
	 AND	dnafrag_end >= '.$bp['start'].' AND dnafrag_id = '.$dnafrag.';';
	 */
...
function getSyntenyGroups($qtldb, $comparadb, $groups1, $groups2, $species_names, $genome_db_ids, $databases){
	...
	//	$dnafragids2 = get_dnafragids($comparadb, $genome_db_ids[1], get_chromo_names_from_group($groups2));
	...
	//warn($synteny1to2);
	/*
	 for ($i = 0; $i < sizeof($bps2); $i++) {
		$synteny_ex2 = getGroupSyntenyIDs($comparadb, $bps2[$i], $dnafragids2[$groups2[$i]['Chr']]);
