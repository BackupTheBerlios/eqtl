 ...
 */
function get_loci_from_sql($databaseTable, $qtldb, $searchType, $chromosomNo, $confidenceInt, $intervalStart  = 0, $intervalEnd  = 0){
				...
				foreach ($loci as $locus) {
					...
					if (($intervalEnd == 0) || ($locusStart <= $intervalEnd[$k]) && ($locusEnd >= $intervalStart[$k])) {
						...
						//check if GroupNo i is the right group (do we have an overlap)
							...
							$group[$i]['loci'][] = $locus[1];
							$group[$i]['end'] = $locusEnd;
						//check if we have a new chromosome
						}elseif ($currentChromosome != $locus[0]) {
