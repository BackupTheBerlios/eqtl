...
if(isset($args[$reg_str])){
	if(isset($args['species'])){
		...
		if($args['species'] == 'Rattus norvegicus'){
			...
			$ex1 =  get_loci_from_sql($database1, $qtldb, 'userinterval', $regionChr, $confidence_int, $intervalStart, $intervalEnd);
			if (!empty($ex1)) {
			...
			$ex2 =  get_loci_from_sql($database2, $qtldb, 'wholeGenome', getChromosoms($compara, 57), $confidence_int);
			...
			// generates an arrays with index -> locinames
			$loci_ex1 = array_map('current',$mapEx1);
			$ex2 =  get_loci_from_sql($database2, $qtldb, 'wholeGenome', getChromosoms($compara, 3), $confidence_int);
			// converts $ex2 in 2 arrays: $groups2 = groupnr -> ('loci' -> lociOfGroup, 'start', 'end') $mapEx2 = index -> (locus,groupNr)
			list($groups2, $mapEx2) = $ex2;
			// generates an arrays with index -> locinames
			$loci_ex2 = array_map('current',$mapEx2);
...
// TODO: more transparency between experiment and species
...
// Loci to genes
//"Mus musculus"
useDB($database1,$qtldb);
$loci2stable_ids_ex1 = loci2stable_ids($loci_ex1,$qtldb);
$group2stable_ids_ex1 = group2stable_ids($mapEx1,$loci2stable_ids_ex1);
//"Rat"
useDB($database2,$qtldb);
$loci2stable_ids_ex2 = loci2stable_ids($loci_ex2,$qtldb);
$group2stable_ids_ex2 = group2stable_ids($mapEx2,$loci2stable_ids_ex2);
