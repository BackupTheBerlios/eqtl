...
function getGroupSyntenyIDs($db, $bp, $genome_db_id){
	$sqlDnafrag = 'select dfr.synteny_region_id from dnafrag_region as dfr inner join dnafrag as df on (dfr.dnafrag_start <='.$bp['end'].' AND
	dfr.dnafrag_end >= '.$bp['start'].' AND dfr.dnafrag_id = df.dnafrag_id AND df.genome_db_id = '.$genome_db_id.');';
...
function getSyntenyGroups($qtldb, $comparadb, $groups1, $groups2, $species_names, $genome_db_ids, $databases){
	// get all synteny ids from groups out of exp. one in a mapping array
	// to later intersect them with the synteny ids from groups out of ex. 2
	$synteny_ex1 = array();
	$result = array();
	useDB($databases[0],$qtldb);
	$bps = groups2bps($qtldb, $groups1, $species_names[0]);
	
	for ($i = 0; $i < sizeof($bps); $i++) {
		$synteny_ex1[$i] = getGroupSyntenyIDs($comparadb, $bps[$i], $genome_db_ids[0]);
		$result[$i] = array();
	}
	// get all synteny ids from exp. two groups and intersect them with the maping array
	// entries out of $synteny_ex1 to determine
	useDB($databases[1],$qtldb);
	$bps = groups2bps($qtldb, $groups2, $species_names[1]);
	for ($i = 0; $i < sizeof($bps); $i++) {
		$synteny_ex2 = getGroupSyntenyIDs($comparadb, $bps[$i], $genome_db_ids[1]);
		for ($j = 0; $j < sizeof($synteny_ex1); $j++) {
			$tempIntersection = array_intersect($synteny_ex1[$j],$synteny_ex2);
			if (!empty($tempIntersection)) {
				$result[$j][] = $i;
			}
		}
