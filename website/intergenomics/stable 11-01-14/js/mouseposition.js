...
function call_detail_view(e) {
	...
	if (oElement.tagName == "TD" && oElement.className == "syn") {
			...
			var ex1 = row.children[0].value;
			var tmp1 = row.parentNode.parentNode;
			var ch = tmp2.getElementsByTagName('thead')[0];
			var ex2 = ch.getElementsByTagName('th')[colIndex].value;
			alert("row: " + ex1 + " col: " + ex2);
	} else {
